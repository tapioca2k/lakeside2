...
namespace Lakeside2
{
    ...
    public abstract class Entity : IDrawable
    {
        ...
        public static Point DIREC_UP = new Point(0, -1);
        public static Point DIREC_RIGHT = new Point(1, 0);
        public static Point DIREC_DOWN = new Point(0, 1);
        public static Point DIREC_LEFT = new Point(-1, 0);
        ...
        /// <returns></returns>
        public static Point getFacingVector(Directions direction)
        {
            switch (direction)
            {
                ...
                default: return Point.Zero; // should never happen
            }
        }
        ...
        /// <param name="v"></param>
        /// <returns></returns>
        public static Directions getDirection(Point v)
        {
        ...
        {
            return new Point((int)location.X / Tile.TILE_SIZE, (int)location.Y / Tile.TILE_SIZE);
        }
        ...
        /// </summary>
        /// <param name="val"></param>
        public void setTileLocation(Point val)
        {
            setLocation(new Vector2(val.X * Tile.TILE_SIZE, val.Y * Tile.TILE_SIZE));
        }
        ...
        public virtual void update(double dt)
        {
            ...
            {
                setDirection(getDirection(moves.Peek()));
                move = moves.Dequeue().ToVector2(); // get next queued move
            }
        ...
        public void queueMove(Point direction)
        {
            moves.Enqueue(new Point(direction.X * Tile.TILE_SIZE, direction.Y * Tile.TILE_SIZE));
            if (follower != null)
            {
                // stand still if entities are overlapping, for now.
                if (follower.getTileLocation() != this.getTileLocation())
                {
                    Point rawFacingDirection = this.getTileLocation() - follower.getTileLocation();
